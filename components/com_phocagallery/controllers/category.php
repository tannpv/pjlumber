...
class PhocaGalleryControllerCategory extends PhocaGalleryController
{
	...
	function remove() {
		...
		if (!empty($catAccess)) {
			$rightDisplayDelete = PhocaGalleryAccess::getUserRight('deleteuserid', $catAccess->deleteuserid, 2, array(0 => 2), $user->get('id', 0), 0);
		}
		// - - - - - - - - - - - - - - - - - - - 	
	...
	*/
	...
	function javaupload() {
		...
		$catAccess	= PhocaGalleryAccess::getCategoryAccess((int)$catid);
		if (!empty($catAccess)) {
			$rightDisplayUpload = PhocaGalleryAccess::getUserRight('uploaduserid', $catAccess->uploaduserid, 2, array(0 => 2), $user->get('id', 0), 0);
		}
		// - - - - - - - - - - - - - - - - - - - - - -	
		// USER RIGHT - FOLDER - - - - - - - - - - - - 		
		$rightFolder = '';
