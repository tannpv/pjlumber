...
class PhocaGalleryViewCategory extends JView
{
	...
	function display($tpl = null) {
		 ...
		 * Note : Get categories   *
		\***************************/
		$categories_option = $this->get('categoriesData');
		/*********** End ***********\
		 * By   : Pham Quang 	   *
		\***************************/
		...
		$font_color 							= $this->params->get( 'font_color', '#b36b00' );
		$background_color 						= $this->params->get( 'background_color', '#fcfcfc' );
		$background_color_hover 				= $this->params->get( 'background_color_hover', '#f5f5f5' );
		...
		$border_color 							= $this->params->get( 'border_color', '#e8e8e8' );
		$border_color_hover 					= $this->params->get( 'border_color_hover', '#b36b00');
		...
		$this->tmpl['overliboverlayopacity']	= $this->params->get( 'overlib_overlay_opacity', 0.7 );
		$margin_box 							= $this->params->get( 'margin_box', 5 );
		$padding_box							= $this->params->get( 'padding_box', 5 );
		...
		$this->tmpl['olfgcolor'], $this->tmpl['olbgcolor'], $this->tmpl['oltfcolor'], $this->tmpl['olcfcolor'],
		$margin_box, $padding_box, $this->tmpl['overliboverlayopacity']));
...
window.addEvent(\'domready\', function(){
	...
	/*	if (!empty ($category->image)) {
		...
		if (!empty($category)) {
			$rightDisplayDelete = PhocaGalleryAccess::getUserRight('deleteuserid', $category->deleteuserid, 2, $user->authorisedLevels(), $user->get('id', 0), 0);
		}
		...
		if (!empty($category)) {
			$rightDisplayUpload = PhocaGalleryAccess::getUserRight('uploaduserid', $category->uploaduserid, 2, $user->authorisedLevels(), $user->get('id', 0), 0);
		}
		...
		if (!empty($category)) {
			$rightDisplay = PhocaGalleryAccess::getUserRight('accessuserid', $category->accessuserid, 0, $user->authorisedLevels(), $user->get('id', 0), $display_access_category);
		}
		if ($rightDisplay == 0) {
		...
		if ($display_back_button == 1) {
			if (!empty($parentCategory)) {
				...
				$rightDisplay = PhocaGalleryAccess::getUserRight('accessuserid', $items[$iS]->accessuserid, $items[$iS]->access, $user->authorisedLevels(), $user->get('id', 0), $display_access_category);
				...
				if ($display_access_category == 1) {
					...
					if (!empty($parentCategory)) {
						$rightDisplayKey = PhocaGalleryAccess::getUserRight ('accessuserid', $items[$iS]->accessuserid, $items[$iS]->access, $user->authorisedLevels(), $user->get('id', 0), 0);
					}
				}
				// - - - - - - - - - - - - - - - - - - - - -
		...
		if ($display_back_button_cv == 1 && $this->tmpl['displaycategoriescv'] == 1) {
			if (!empty($parentCategory)) {
				...
				// USER RIGHT - ACCESS - - - - - - - - - - -
				// Should be the link to parentcategory displayed
				$rightDisplay = PhocaGalleryAccess::getUserRight('accessuserid', $itemsCV[$iCV]->accessuserid, $itemsCV[$iCV]->access, $user->authorisedLevels(), $user->get('id', 0), $display_access_category);
				...
				if ($display_access_category == 1) {
					...
					// so we get rightDisplayKey = 0 then the key will be displayed
					if (!empty($parentCategory)) {
						$rightDisplayKey = PhocaGalleryAccess::getUserRight ('accessuserid', $itemsCV[$iCV]->accessuserid, $itemsCV[$iCV]->access, $user->authorisedLevels(), $user->get('id', 0), 0);
					}
				}
				// - - - - - - - - - - - - - - - - - - - - -
			...
			if (!empty($subCategory)) {
				...
				for($iSub = 0; $iSub < $totalSubCat; $iSub++) {
					...
					if ($rightDisplay > 0) {
						...
						$extImage = PhocaGalleryImage::isExtImage($items[$iCV]->extid);
						if ($extImage) {
		...
		for($iM = 0; $iM < $totalImg; $iM++) {
			...
			if ($this->tmpl['switchimage'] == 1) {
				if ($basicImageSelected == 0) {
					...
					if (isset($items[$iS]->extpic) && $items[$iS]->extpic != '') {
						...
						$this->tmpl['basicimage']	= JHTML::_( 'image.site', str_replace('phoca_thumb_m_','phoca_thumb_l_',$items[$iS]->linkthumbnailpath), '', '', '', '', $wHString);
			...
			$items[$iS]->Itemid = JRequest::getVar('Itemid', 0, '', 'int');
		...
		if ((int)$this->tmpl['displayrating'] == 1) {
			...
			// Already rated?
			$this->tmpl['alreadyrated']	= PhocaGalleryRateCategory::checkUserVote( (int)$id, (int)$user->id );
		}
		...
		if ((int)$this->tmpl['displaycomment'] == 1) {
			...
			$this->tmpl['alreadycommented'] 	= PhocaGalleryCommentCategory::checkUserComment( (int)$id, (int)$user->id );
		...
		$this->assignRef( 'categories_option' ,	$categories_option);// Category View
